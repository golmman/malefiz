...
impl Display for Piece {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let displayed_text = match self {
            Piece::RED => "\x1b[31m1\x1b[0m",
            Piece::GREEN => "\x1b[32m2\x1b[0m",
            Piece::YELLOW => "\x1b[33m3\x1b[0m",
            Piece::BLUE => "\x1b[34m4\x1b[0m",
            Piece::BARRICADE => "\x1b[37m@\x1b[0m",
            Piece::NONE => "-",
        };
...
impl Display for Board {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut board_text = BOARD.to_string();

        board_text = board_text.replace("    ", "  ");

        for (index, board_space) in self.board_spaces.iter().enumerate() {
            let index_padded = format!("{:0>3}", index);
            let piece_formatted = format!("{}", board_space.piece);
            board_text = board_text.replace(&index_padded, &piece_formatted);
        }
